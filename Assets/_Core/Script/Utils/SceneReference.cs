...
{
    //是否显示额外的 GUI
    //是否加入 Building List, 各种按钮之类的
    private static bool DRAW_EXTRA_GUI = false;
    ...
    static readonly RectOffset boxPadding = EditorStyles.label.padding;
    static readonly float padSize = 0f;
    static readonly float lineHeight = EditorGUIUtility.singleLineHeight;
    static readonly float paddedLine = lineHeight + padSize;
    static readonly float footerHeight = 0f;
    ...
    private void DrawSceneInfoGUI(Rect position, BuildUtils.BuildScene buildScene, int sceneControlID)
    {
        ...
        if (DRAW_EXTRA_GUI == false) return;
